...
| 2026-04-28 | REQ-S76-001 | Remove runtime demo KG wiring and prove the API is hitting the real production database, not local demo/archived/empty data. | DONE | `kg-api-server.py` path guard, explicit-only `docker-compose.yml`, deleted local archived/demo KG artifacts, `scripts/_lib/prod_kg_client.py` self-test against `100.88.170.57:8400` |
...
| 2026-04-28 | PROMPT-S76-001 | Real KG runtime proof | `./.venv/bin/python scripts/_lib/prod_kg_client.py` + `curl http://100.88.170.57:8400/api/v1/{health,debug/paths,search}` | Confirms live prod path `/home/kg/cognebula-enterprise/data/finance-tax-graph`, Lance path `/home/kg/data/lancedb`, search hit `TT_VAT`, 368,910 nodes, 1,014,862 edges |
...
| Why was local packaged health previously `kuzu=false`? | `docker-compose.yml` was mounting the empty directory `data/finance-tax-graph/` into a path that Kuzu expects to be a database file. The later archived-baseline default has now been removed too: packaged API startup requires explicit real `COGNEBULA_GRAPH_PATH` + `COGNEBULA_LANCE_PATH`, and `kg-api-server.py` refuses demo/archived/missing/empty DB paths. |
...
| How can we get a richer local graph without modifying the archived baseline file? | Do not recreate a local demo graph for runtime work. Local development should call the real KG through `COGNEBULA_KG_URL=http://100.88.170.57:8400`; CI/unit tests should seed temporary Kuzu fixtures inside the test itself. |
...
| How do we know the current API examples are not just textually correct? | Because `scripts/_lib/prod_kg_client.py` and direct `curl` calls hit the real Tailscale API (`100.88.170.57:8400`), returning live health, production DB paths, `TT_VAT` search hits, and 368,910 / 1,014,862 quality metrics. |
