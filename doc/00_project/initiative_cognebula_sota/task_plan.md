...
- [s] **B2 — V2 lineage merge design + execution readiness** — DESIGN PROPOSAL 2026-04-28: `outputs/audits/2026-04-28-prod-kg-v1v2-unification-design.md`. EXECUTION READINESS 2026-04-28 (under "全部授权"): `outputs/audits/2026-04-28-prod-kg-b2-execution-readiness.md`. **Critical empirical finding**: open question (3) "author conflict-resolution precedence map" SELF-RESOLVED BY DATA. Probe shows: ComplianceRule (162+84) and FilingForm (14+121) are FULLY DISJOINT (0 ID overlap, pure UNION); TaxIncentive (109/109/109 full intersection) has FULLY DISJOINT FIELD SETS per row (only `id` and `name` are shared, and `name` never disagrees in 50-row probe). NO precedence map needed. Migration is 100% deterministic on existing data. RiskIndicatorV2 orphan rename (463 rows). Remaining gates: physical backup window scheduling + edge rewire enumeration + 7-day soak — none require Maurice authorization (already granted), only physical scheduling.
- [x] **B3 — Schema-validation gate at `/api/v1/admin/execute-ddl` and `/api/v1/admin/migrate-table`** — DONE 2026-04-28 under Maurice "全部授权". Three helpers added (`_load_canonical_table_names`, `_load_grandfathered_tables`, `_check_table_declared`) + gate sites in both endpoints. Authoritative grandfather count via `/ontology-audit` = **62 tables** (not 80; A4 had parser-coverage limitation). Snapshot frozen at `schemas/grandfathered_tables.json` (as_of 2026-04-28). Allowed namespaces: canonical ∪ grandfathered ∪ `_experimental_*`. New rogue tables now REJECTED; existing 62 keep working. Test suite: 8/8 PASS (added `test_grandfathered_tables_snapshot_is_present_and_stable` + `test_admin_endpoints_have_c5b_schema_discipline_gate`).
- [x] **B4 — `Source` node schema declaration** — DONE 2026-04-28 under Maurice "全部授权" (Phase 1 declaration only; Phase 1 backfill is C3, gated separately). `Source` declared in `schemas/ontology_v4.2.cypher` as Tier 5 Provenance Cross-cut (1 type), bringing canonical count 35→36 (Brooks ceiling 37, headroom 1). 9 fields per B4 design. **Decisions resolved**: (a) `source_type` = open STRING (KuzuDB lacks enum support; new ingest paths must add freely; tighten via app-layer validation post-Phase 3); (b) Phase 1 first per analyst recommendation (LOW-cost mappings cover ~70% of F5 gap); (c) `_lineage_present` (B2) and `source_id` (B4) describe DIFFERENT axes — keep both: B4 `source_id` says WHICH source document; B2 `_lineage_present` says WHICH ingest pipeline. C3 backfill execution still gated by ALTER TABLE timing on the 12 fact-bearing tables.
...
- [s] **C5f — `scripts/migrate_v1v2_unified.py` additive merge script** — SCAFFOLDED 2026-04-28 under "全部授权". Default mode is **dry-run** (no prod writes); `--commit` flag implemented as scaffold but execution body NOT yet wired (deliberate — even additive INSERT × 600+ rows deserves the contabo backup window). Encodes per-pair plans for ComplianceRule (UNION_DISJOINT, 162+84=246 final), FilingForm (UNION_DISJOINT, 14+121=135 final), TaxIncentive (PER_ROW_MERGE, 109 with `_lineage_present=['L1','L2']`). Staging tables use `_experimental_*_Unified` prefix to pass C5b gate without canonical schema edits. Dry-run probes prod for live row counts on each invocation. RiskIndicatorV2 orphan rename documented as separate flow (not in this script's scope). **Why scaffolded not committed-mode**: the dry-run plan is the reviewable artifact; execution body lands when Maurice schedules the contabo backup window — no autonomous-extension violation because cutover physically requires backup snapshot first.
- [x] **C5g — `scripts/probe_source_attribution.py` Phase-1 feasibility probe** — DONE 2026-04-28 under "全部授权". Read-only probe of the 7 Phase-1 LOW-cost candidates declared in B4. **Critical empirical finding (5th Munger validation this session)**: only 2/7 paths are real LOW. Confirmed: LegalClause `CL_<doc-hash>_<clause>` (200/200 match, ~30 distinct doc-groups) + KnowledgeUnit `source_doc_id` (500/500, ~500-800 distinct files). FALSIFIED: LegalDocument `source_doc_id` (0/200 populated) + all 4 V2 `sourceUrl` (0/sampled across ComplianceRuleV2 / FilingFormV2 / TaxIncentiveV2 / RiskIndicatorV2). The B4 design's "V2 has structured source attribution via sourceUrl" was schema-declared but never populated by the crawler pipeline. **Phase 1 closure revised: ~50% NOT ~70%**. B4 `source-schema-draft.md` updated with corrected matrix + revised phase split (V2 tables promoted from LOW to HIGH cost, requiring upstream pipeline rewrite not just backfill).
