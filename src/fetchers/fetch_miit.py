"""Fetcher: Ministry of Industry and Information Technology (miit.gov.cn) news and policies.

NOTE: MIIT uses Hanweb CMS which renders listing content via client-side JavaScript.
...
log = logging.getLogger("fetch_miit")
...
# Sections: homepage news links (gxdt = work dynamics under xwfb)
SECTIONS = [
    ("work_dynamics", f"{BASE_URL}/xwfb/gxdt/"),
    ("policy_interpret", f"{BASE_URL}/zwgk/zcjd/"),
]
...
def _parse_listing(html: str) -> list[dict]:
    """Extract links from MIIT listing pages.

    MIIT uses /xwfb/gxdt/art/YYYY/art_UUID.html pattern.
    Also matches /zwgk/zcjd/art/YYYY/art_UUID.html.
    """
    items = []
    seen_urls = set()
    ...
    for a_tag in soup.find_all("a", href=True):
        ...
        if not title or len(title) < 8:
            continue
        # Match MIIT content URLs: /.../art/YYYY/art_UUID.html
        if re.search(r"/(?:xwfb|zwgk)/\w+/art/\d{4}/art_[a-f0-9\-]+\.html", href, re.IGNORECASE):
    ...
    if len(items) < 3:
        for m in re.finditer(
            r'<a[^>]*href="([^"]*?/(?:xwfb|zwgk)/\w+/art/\d{4}/art_[a-f0-9\-]+\.html)"[^>]*>([^<]{8,})</a>',
            html,
            re.IGNORECASE,
        ):
...
def _fetch_detail(client: httpx.Client, url: str) -> tuple[str, str]:
    ...
    try:
        ...
        date_str = ""
        date_el = soup.select_one("span.time, span.info-time, div.article-info, div.cinfo")
        if date_el:
...
def fetch(output_dir: str, fetch_detail: bool = True) -> list[dict]:
    ...
    if output_dir:
        ...
        out_path = os.path.join(output_dir, "miit.json")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch MIIT news and industry policies")
