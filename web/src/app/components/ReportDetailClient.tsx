...
export function ReportDetailClient({ report: r }: { report: Report }) {
  const [status, setStatus] = useState(r.status);
  const [isApproved, setIsApproved] = useState(r.status === "reviewed");
  ...
  const statusText = status === "ai" ? "AI 生成" : status === "reviewed" ? "已人工复核" : "需关注";

  function handleApprove() {
    setStatus("reviewed");
    setIsApproved(true);
    toast("报告已批准，状态已更新为「已人工复核」", "success");
  }
      ...
      >
        <div>
          ...
          <p style={{ fontSize: 12, color: "var(--color-text-tertiary)", marginTop: 4 }}>
            {statusText} · {r.date} · &yen;{r.amount}
          </p>
        </div>
