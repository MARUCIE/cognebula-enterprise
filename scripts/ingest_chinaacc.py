...
def safe_create_ku(node_id, title, content, source):
    ...
    try:
        # Check if exists
        r = conn.execute(
            "MATCH (k:KnowledgeUnit {id: $id}) RETURN k.id", {"id": node_id}
        )
        if r.has_next():
            ...
            # Create new
            conn.execute(
                "CREATE (k:KnowledgeUnit {id: $id, title: $t, content: $c, source: $s, extracted_by: $eb})",
                {"id": node_id, "t": title[:200], "c": content, "s": source, "eb": "ingest_chinaacc-v1"},
            )
            writes += 1
            if writes % 50 == 0:
                conn.execute("CHECKPOINT")
...
r = conn.execute(
    "MATCH (k:KnowledgeUnit) WHERE k.content IS NOT NULL AND size(k.content)>=100 RETURN count(k)"
)
before = r.get_next()[0]
...
if os.path.exists(chinaacc_path):
    ...
    for item in data:
        ...
        iid = (
            item.get("id", "")
            or hashlib.sha256(f"chinaacc:{title}".encode()).hexdigest()[:16]
        )
...
if os.path.exists(doctax_path):
    ...
    for item in data:
        ...
        iid = (
            item.get("id", "")
            or hashlib.sha256(f"doctax:{title}".encode()).hexdigest()[:16]
        )
