"""Fetcher: China Accounting Standards Committee (casc.org.cn) accounting standards and interpretations."""
...
log = logging.getLogger("fetch_casc")
...
BASE_URL = "http://www.casc.org.cn"
# Main listing page (single section -- accounting standards body)
SECTIONS = [
    ("accounting_standards", f"{BASE_URL}/"),
]
...
def _parse_listing(html: str) -> list[dict]:
    """Extract links from CASC main page.

    CASC uses /YYYY/MMDD/ID.shtml pattern.
    """
    items = []
    seen_urls = set()
    ...
    for a_tag in soup.find_all("a", href=True):
        ...
        if not title or len(title) < 8:
            continue
        # Match CASC content URLs: /YYYY/MMDD/ID.shtml
        if re.search(r"/\d{4}/\d{4}/\w+\.shtml", href):
...
def _fetch_detail(client: httpx.Client, url: str) -> tuple[str, str]:
    ...
    try:
        ...
        date_str = ""
        date_el = soup.select_one("span.time, span.info-time, div.article-info, div.source")
        if date_el:
...
def fetch(output_dir: str, fetch_detail: bool = True) -> list[dict]:
    ...
    if output_dir:
        ...
        out_path = os.path.join(output_dir, "casc.json")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch CASC accounting standards and interpretations")
