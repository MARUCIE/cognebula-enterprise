"""Fetcher: National Bureau of Statistics (stats.gov.cn) latest releases and data analysis."""
...
log = logging.getLogger("fetch_stats")
...
# Sections: latest releases, data interpretation
SECTIONS = [
    ("latest_release", f"{BASE_URL}/sj/zxfb/"),
    ("data_analysis", f"{BASE_URL}/sj/sjjd/"),
]
...
def _parse_listing(html: str, section_url: str = "") -> list[dict]:
    """Extract links from stats.gov.cn listing pages.

    Stats.gov.cn uses relative paths: ./YYYYMM/tYYYYMMDD_ID.html
    """
    items = []
    seen_urls = set()
    ...
    for a_tag in soup.find_all("a", href=True):
        ...
        if not title or len(title) < 8:
            continue
        # Match stats content URLs: any path with /YYYYMM/tYYYYMMDD_ID.html
...
def _fetch_detail(client: httpx.Client, url: str) -> tuple[str, str]:
    ...
    try:
        ...
        date_str = ""
        date_el = soup.select_one("span.time, span.info-time, div.article-info, div.pub-info")
        if date_el:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fetch NBS latest releases and data analysis")
